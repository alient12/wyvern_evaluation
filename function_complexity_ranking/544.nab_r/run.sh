...
# === Config ===
BIN="/usr/cpu2017/benchspec/CPU/544.nab_r/exe/nab_r_base.wyvern-m64"
INPUT=(
    hkrdenq
    1930344093
    1000
...
/magic-trace run/    {n3++; s3+=$NF}
/valgrind run/       {n4++; s4+=$NF}

END {
    if (n0) print "VANILLA avg:", s0/n0, "s"
    if (n1) print "WYVERN NORMAL avg:", s1/n1, "s"
    if (n2) print "WYVERN STRESS avg:", s2/n2, "s"
    if (n3) print "magic-trace avg:", s3/n3, "s"
    if (n4) print "valgrind avg:", s4/n4, "s"
}
' "$TIME_LOG" >> "$TIME_LOG"